...
const model=createServer(async(req,res)=>{try{
 ...
 if(live)output=await live.complete(input);
  ...
  let name,args,final;
  if(task.includes('请记住：'))final='已记住项目代号青竹，汇报偏好为先结论、后证据。';
...
try{
 ...
 if(verify){
  assert.equal(state.status,'ready');assert.equal(state.errors.length,0);assert.ok(state.items.some(i=>i.origin==='openclaw'&&i.kind==='plugin'));assert.ok(state.items.some(i=>i.name==='dsh_grep'));
  const completed=[];
  for(const [loop,message] of [['dsh','使用 OpenClaw 的 read 读取 evidence.txt，再用 DSH 的 dsh_grep 搜索 CoNest，汇总两次工具返回的真实内容。'],['dsh','请记住：CoNest 演示的项目代号是青竹，汇报偏好为先结论、后证据。请确认记忆。'],['openclaw','刚才记住的 CoNest 项目代号和汇报偏好是什么？请根据共享记忆回答。'],['openclaw','使用 OpenClaw 的 read 读取 evidence.txt，再用 DSH 的 dsh_grep 搜索 CoNest，汇总两次工具返回的真实内容。']]){
  ...
  assert.equal(activity.memory.length,1,'Recall questions must not be captured as memories');
  assert.ok(activity.memory.some(m=>m.includes('青竹')));assert.ok(completed[2].text.includes('青竹'));
  await writeFile(path.join(demoRoot,'acceptance.json'),JSON.stringify({at:new Date().toISOString(),mode:live?'live':'fixture',passed:true,completed,activity,requests,usage:live?.report()},null,2),{mode:0o600});
