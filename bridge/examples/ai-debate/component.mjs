...
function callDeepSeek(apiKey, messages, temperature = 0.8, maxTokens = 800) {
...
export default {
  ...
  apply(ctx, config) {
    ...
    ctx.bridgeCapabilities.register(ctx, 'ai_debate', async (args, invocation) => {
      ...
      if (!apiKey) {
        return { topic, rounds, error: 'No DeepSeek API key configured' };
      }
...
- This is round ${r}. Previous debate history:\n${history.map(h => `[${h.side}] ${h.text}`).join('\n') || '(none)'}`,
        ...
        const proText = await callDeepSeek(apiKey, proPrompt, 0.8, 600);
...
- This is round ${r}. Previous debate history:\n${history.map(h => `[${h.side}] ${h.text}`).join('\n') || '(none)'}`,
        ...
        const conText = await callDeepSeek(apiKey, conPrompt, 0.8, 600);
...
Be concise and specific.`,
      ...
      const verdictText = await callDeepSeek(apiKey, judgePrompt, 0.3, 500);
