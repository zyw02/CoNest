...
Run `flow.py --config /var/lib/conest-agent/config.json --pr 1` for a selected PR or omit `--pr` for the queue. `--intake` only updates queue receipts. `--retry --pr 1` starts a fresh attempt for a stopped revision after resolving its blocker. The supplied systemd service/timer files use these fixed paths, a 30-second polling interval, one worker, a 4 GiB worker memory ceiling and a 30-call daily model budget. Install them under `/etc/systemd/system`, then enable `conest-agent.timer` and `conest-agent-intake.timer`. Stop both timers and `conest-agent.service` to stop automation. Inspect progress with `journalctl -u conest-agent.service` and the per-PR JSON under `/var/lib/conest-agent/state`. SQLite checkpoints and detailed logs are local operator records and must not be committed or attached to PRs.
