...
`CoNest Review` performs deterministic diff checks, repository/type checks and behavior tests with read-only PR permissions. A separate `workflow_run` job reads only its generated artifact and maintains one `github-actions` review comment. Treat blocker findings as required fixes or explain a false positive in the PR; warnings guide human review. This deterministic bot does not approve or merge PRs. The separately operated CoNest Agent below can take over repairs and merge only after its configured validation gates pass.

The optional **CoNest Agent** uses LangGraph with persistent SQLite checkpoints to call the operator's Codex CLI (`gpt-6-sol`, high reasoning). PR events publish a metadata-only queue receipt; a local timer provides a fallback and a separate worker handles the queue. All PRs share one dedicated checkout, with one task branch active at a time. The operator's development checkout is separate. The controller and its Python environment live outside that checkout so contributor branches cannot replace the running service.

Repair workers can build and run a selected test file through the isolated test tool; review workers retain read-only source access. An interrupted task retains ownership of the shared workspace until it resumes or its edits are saved. The graph integrates the current target branch, reviews source, assigns repairs to the configured maintainer, fixes bounded findings, runs isolated checks, re-reviews, pushes to the original PR branch and waits for named GitHub checks before squash merging. Contributors must allow maintainer edits. Repair commits retain the existing history; the squash message includes verified contributor identities as `Co-authored-by` trailers. Model findings alone are insufficient to merge. Changed revisions, incomplete source coverage, failed tests, policy changes or an exhausted repair budget stop the run. A paused CI wait resumes from its checkpoint. Stopped task edits are retained as a local branch or recovery patch before the workspace is released. Unexpected workspace changes prevent switching to another task.

Controller source and pinned Python dependencies are in `scripts/agent-review/`. Run `/path/to/venv/bin/python -m unittest discover -s scripts/agent-review -p 'test_*.py'` to verify the lifecycle without model calls or GitHub writes. Create a separate environment with `python3 -m venv /var/lib/conest-agent/venv` and install `scripts/agent-review/requirements.txt`. Install the Node and pnpm versions pinned by this repository under `/var/lib/conest-agent/toolchain`, plus Linux bubblewrap. Authenticate `codex login` and `gh auth login` as the operator, clone the repository once to `/var/lib/conest-agent/workspace`, copy the Python controller files to `/var/lib/conest-agent/app`, and adapt `config.example.json` to `/var/lib/conest-agent/config.json`. The example disables publishing and merging; enable them only for the intended repository. Keep credentials outside the checkout.

Run `flow.py --config /var/lib/conest-agent/config.json --pr 1` for a selected PR or omit `--pr` for the queue. `--intake` only updates queue receipts. `--retry --pr 1` starts a fresh attempt for a stopped revision after resolving its blocker. The supplied systemd service/timer files use these fixed paths, a 30-second polling interval, one worker, a 4 GiB worker memory ceiling and a daily limit of 30 Codex executions (each execution may make multiple model requests). Install them under `/etc/systemd/system`, then enable `conest-agent.timer` and `conest-agent-intake.timer`. Stop both timers and `conest-agent.service` to stop automation. Inspect progress with `journalctl -u conest-agent.service` and the per-PR JSON under `/var/lib/conest-agent/state`. SQLite checkpoints and detailed logs are local operator records and must not be committed or attached to PRs.
